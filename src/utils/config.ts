...
export const getStoryFeaturedImage = (story: { image_url?: string | null; middle_images?: string[] | null; cuentito_uid?: number | null }) => {
  if (story.image_url) return story.image_url;
  if (story.middle_images && story.middle_images.length > 0) return story.middle_images[0];
  return null;
};
